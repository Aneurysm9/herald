...
impl DnsServer {
    ...
    /// Evaluate RFC 2136 §3.2 prerequisites against the current zone contents.
    ///
    /// Fetches the current records from the backend that owns the zone
    /// declared in the zone section, then checks each prerequisite RR
    ...
    async fn evaluate_prereqs(
        ...
        // Find the backend for this zone and fetch current records.
        let (_, backend_idx) = derive_zone(zone_name_trimmed, &self.backends)
            .map_err(|_| DnsError::NotZone(format!("no backend for zone {zone_name}")))?;
        let existing = self.backends[backend_idx]
            .get_records()
            .await
            .map_err(|e| DnsError::Refused(format!("failed to fetch zone records: {e}")))?;
        ...
        for prereq in prereqs {
            ...
            match (prereq_class, prereq_type, has_rdata) {
                ...
                (DNSClass::ANY, rtype, false) if rtype != RecordType::ANY => {
                    ...
                    let found = existing.iter().any(|r| {
                        r.record.name == prereq_name && r.record.value.type_str() == type_str
                    });
                    if !found {
                ...
                (DNSClass::IN, rtype, true) if rtype != RecordType::ANY => {
                    ...
                    let found = existing.iter().any(|r| {
                        r.record.name == prereq_name
                            && r.record.value.type_str() == type_str
                            && r.record.value == prereq_value
                    });
                    if !found {
                        return Err(DnsError::PrereqFailed(ResponseCode::NXRRSet));
                    }
                ...
                (DNSClass::ANY, RecordType::ANY, false) => {
                    let found = existing.iter().any(|r| r.record.name == prereq_name);
                    if !found {
